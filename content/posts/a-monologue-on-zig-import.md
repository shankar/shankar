...
When we need access to the declarations in another file we import that file using the `@import` function. `@import` function takes a filepath or package name string literal as argument and returns a `struct type`. Filepath has to be relative to the file using the `@import` function , **the file path cannot reach above the directory containing the root file**. The root file is the entry point file to the program or library.

Providing a constant as argument for @import function will throw an error even though the constant is a comptime known value. It can accept only string literal.
...
* builtin - Compiler and target info package
* root - root source file as package

We all know the standard library package . It is a collection of quality functions and declarations for a wide range of use cases so that we can use them without having to start from scratch.

The builtin package is imported using `@import("builtin")`. It is generated by the compiler and contains compile time constants. We can output the source code of this package using the command `zig build-exe --show-builtin`.
Mind you there is a builtin sub package in standard library which exposes the language interfaces and this sub package is imported using `@import("std").builtin`.

Now lets talk about the root package, it is usually used by library so that the library has access to the declarations in the root source file. Lets say the library wants to see if `std_options` has been set and use it instead of the default, the code will be like:

```zig
const root = @import("root");

// The @hasDecl funtion checks if the second option is declared in a container which is
// the first option. The @hasDecl function requires the std_options declaration to be
// exposed by the root file using the pub keyword.
pub const options: Options = if (@hasDecl(root, "std_options")) root.std_options else .{};
```
So that is all we need to know about the 3 packages that are availabe by default.

Now remember @import function returns a struct ? let us write some code to demonstrate that.
{{< file "hello.zig" >}}
```zig
